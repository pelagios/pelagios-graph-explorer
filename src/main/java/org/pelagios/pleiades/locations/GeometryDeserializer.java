...
public class GeometryDeserializer implements JsonDeserializer<Geometry> {
	...
	public Geometry deserialize(JsonElement json, Type typeOfT,
		...
		geometry.setType(org.pelagios.pleiades.locations.Geometry.Type.valueOf(type.getAsString().toUpperCase()));

		// The geometry
		if (geometry.getType() == org.pelagios.pleiades.locations.Geometry.Type.POINT) {
			geometry.setShape(deserializePoint(obj.get(COORDINATES)));
		} else if (geometry.getType() == org.pelagios.pleiades.locations.Geometry.Type.POLYGON) {
